...
export default function DashboardPanel({ jugadorId }) {
  ...
  const totalGoles = filteredMatches.reduce((acc, m) => acc + m.goals, 0);
  const totalAsistencias = filteredMatches.reduce((acc, m) => acc + m.assists, 0);
  const avgRating = filteredMatches.length > 0
    ? (filteredMatches.reduce((acc, m) => acc + m.rating, 0) / filteredMatches.length).toFixed(2)
    : 0;
