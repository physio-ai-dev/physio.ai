  ...
  useEffect(() => {
    const cargarDatosDashboard = async () => {
      ...
      try {
        ...
        setStats(dataStats);
        setAnalisis(dataIA.reporte || dataIA.resultado || dataIA.analysis || "");
      } catch (err) {
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    ...
                    <TableRow>
                      ...
                      <TableCell align="right" sx={{ color: "primary.main", fontWeight: 800, borderBottom: "1px solid rgba(255, 255, 255, 0.05)" }}>
                        {filteredMatches.length > 0 ? totalGoles : (stats?.goles ?? 0)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell sx={{ borderBottom: "1px solid rgba(255, 255, 255, 0.05)" }}>Asistencias</TableCell>
                      <TableCell align="right" sx={{ color: "primary.main", fontWeight: 800, borderBottom: "1px solid rgba(255, 255, 255, 0.05)" }}>
                        {filteredMatches.length > 0 ? totalAsistencias : (stats?.asistencias ?? 0)}
                      </TableCell>
                    </TableRow>
                    <TableRow>
          ...
          <Card variant="outlined" sx={{ height: "100%", border: "1px solid rgba(16, 185, 129, 0.25)", borderRadius: 4, bgcolor: "rgba(16, 185, 129, 0.01)" }}>
            <CardContent sx={{ p: 3 }}>
              ...
              <Typography variant="body2" sx={{ lineHeight: 1.8, whiteSpace: "pre-line", color: "text.secondary", fontWeight: 500 }}>
                {analisis || "No hay informe predictivo disponible para este jugador."}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
